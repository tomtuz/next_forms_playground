import { parserVue, pluginVue, TSPlugin } from '@/plugins'
...
export const reactivityTransform: Linter.Config[] = [
  {
    ...
    name: 'eslint/vue/reactivity-transform',
    plugins: {
      vue: pluginVue,
    },
    rules: {
...
const vueTs: Linter.Config[] = typescriptCore
  ...
  .map((config) => {
    return {
      ...config,
      files: [GLOB_VUE],
      name: `eslint/vue/${config.name?.replace('eslint/', '') || 'anonymous'}`,
    }
  })
...
export const vue: Linter.Config[] = [
  ...vueTs,
  {
    ...
    name: 'eslint/vue',
    plugins: {
      '@typescript-eslint': TSPlugin.plugin as any,
      vue: pluginVue,
    },
